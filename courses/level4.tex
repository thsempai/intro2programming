...
Pour définir une fonction, il faut simplement écrire \codeintext{def} suivit de parenthèse\footnote{Nous verrons dans la suite du cours que ces parenthèses ne seront pas toujours vide.} et d'un "\codeintext{:}
...
Dans le jeu du bingo (Stage 3-4) Ajouter une \emph{fonction} \codeintext{bingo} qui affiche "Bingo!" dans la console.
...
Par exemple, le nombre aléatoire donné par la focntion \codeintext{randint} est son \emph{retour}.
...
1 et 6 sont les \emph{paramêtres} de \codeintext{randint}.
...
Dans cette exemple un peu bateau, \codeintext{number} est l'argument de la \emph{fonction} \codeintext{double}.
...
Dans cette autre exemple, \codeintext{array} et \codeintext{number} sont les arguments de la fonction \codeintext{list\_multiplication}
...
Le code affichera si le nombre est plus petit ou plus grand, et renverra \codeintext{True} si le joueur à deviner juste et \codeintext{False} dans les autre cas.
...
Dans le script ci-dessus, l'\emph{argument} à une \emph{valeur par défaut} de 6.
Si le code appelant est:
\begin{lstlisting}
result = dice_simulation(10)
\end{lstlisting}

le \emph{paramêtre} \codeintext{faces} vaut 10, mais si on ne l'avait pas préciser, comme dans le code suivant:

\begin{lstlisting}
result = dice_simulation()
\end{lstlisting}

le \emph{paramêtre} \codeintext{faces} vaut 6 dans ce cas là.
...
La première fois, le nombre est choisis de manière normale, mais à chaque fois que le joueur recommence de jouer, la borne maximum est modifiée de manière à valoir \codeintext{born\_max + (7 - tentatives}.
